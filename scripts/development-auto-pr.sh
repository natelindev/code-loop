...
WORKSPACE_ROOT="$HOME/codeloop-workspaces"
...
load_config() {
  ...
  WORKSPACE_ROOT="${WORKSPACE_ROOT:-$HOME/codeloop-workspaces}"
...
notify_success() {
  local url="$1"
  osascript -e "display notification \"PR ready: $url\" with title \"CodeLoop\"" >/dev/null 2>&1 || true
...
notify_error() {
  local msg="$1"
  osascript -e "display notification \"$msg\" with title \"CodeLoop\"" >/dev/null 2>&1 || true
  if [[ "$NOTIFICATION_SOUND" == "true" ]]; then
    afplay /System/Library/Sounds/Glass.aiff >/dev/null 2>&1 || true
  fi
}
...
Output ONLY the raw commit message text. No markdown, no backticks, no quotes, no preamble, no explanation."
  ...
  if [ -z "$commit_msg" ]; then
    commit_msg="chore: apply codeloop updates"
  fi
...
BODY: <body>"
  ...
  if [ -z "$title" ]; then
    title="chore: codeloop update"
  fi
  if [ -z "$(echo "$body" | trim)" ]; then
    body="Automated update generated by CodeLoop."
  fi
...
main() {
  ...
  log "INIT" "Starting CodeLoop pipeline"
  log "INIT" "Repo: $REPO"
  ...
  if run_pipeline; then
    ...
    cleanup_plan_file
    notify_error "CodeLoop run failed. Check log: $LOG_FILE"
    exit 1
  fi
}
